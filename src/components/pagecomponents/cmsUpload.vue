<template>
    ...
    <!-- 水印 -->
      <el-checkbox v-model="mark"  class="mark-box" v-if="isMark">水印</el-checkbox>
    </div>
</template>
  ...
  watch: {
    src(curVal, oldVal) {
      ...
      if (curVal.indexOf("http") >= 0) {
        ...
        this.imageUrl = curVal ? this.$getUrl() + curVal : "";
      }
      // if(curVal!=''){
