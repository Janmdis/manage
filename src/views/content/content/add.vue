    ...
    <div class="cms-content-right">
      <el-form ref="form" :model="info" :rules="rules" class="cms-form" label-width="162px" v-loading="loading"
        ...
        <el-form-item v-for="(item,index) in itemList" :key="index"
                      ...
                      :prop="item.field"
                      v-if="item.field!='channelId'&&item.field!='title'&&item.field!='typeImg'&&item.display">
          <!-- 文本 -->
          ...
          <!-- 标题，内容图片 类型图 -->
          <cms-upload :src="info[item.field]" :field="item.field" v-if="item.field=='titleImg'&&hasTitleImg"
                      :isMark="false"
                      @get="getFieldImg"></cms-upload>
          <cms-upload :src="info[item.field]" :field="item.field" v-if="item.field=='contentImg'&&hasContentImg"
                      :isMark="false"
                      @get="getFieldImg"></cms-upload>
          <!-- 附件 -->
