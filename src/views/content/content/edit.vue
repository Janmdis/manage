  ...
  <section class="cms-body flex">
    ...
    <div class="cms-content-right">
      <el-form ref="form" :model="info" :rules="rules" class="cms-form" label-width="162px" v-loading="loading">
        ...
        <!-- 系统字段 -->
        <el-form-item v-for="(item,index) in itemList" :key="index" :label="item.label" :class="[item.single?'flex-100':'flex-50']" :prop="item.field" v-if="item.field!='channelId'&&item.field!='title'&&item.field!='typeImg'&&item.display">
          <!-- 文本 -->
          ...
          <!--文档路径  -->
          <cms-attach-upload :src="info['docPath']" v-if="item.field=='docPath'" :multiple="false" :isDoc="true" @change="getDocPath"></cms-attach-upload>
          <!-- 图片集 -->
          ...
          <div v-if="item.field=='typeId'">
            ...
            <div v-if="isTypeImg" class="bread-box" style="margin-top:12px;">
              <label style="margin-right:10px;">类型图</label>
...
export default {
  ...
  methods: {
    ...
    getDefaultInfo(id) {
      ...
      this.info.channelIds = this.contentInfo.channelIds.sort();
